...
for repo in repos:
    ...
    if 'clones' in traffic_data:
        ...
        if last_clone:
            result['message'] = f"*{repo_name}: {last_clone['timestamp']}"
            ...
            result['message'] = f"Repo: {repo_name} has no cloning data."

    else:
        result['message'] = f"Repo: {repo_name} - no traffic data available."
